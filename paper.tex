...
\emph{Schema evolution} is a roadblock to feedback in many situations that deserves greater attention.
When requirements on the shape of data change then existing data must be migrated into the new shape, and existing code must be modified to suit. In many cases these adaptations must be performed manually, or with ad hoc code. Manual schema evolution not only delays feedback but since it occurs outside the purvey of version control tools it also interrupts collaboration.
