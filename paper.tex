...
\cite{localfirst}). To provide immediate feedback, future programming systems will also need to
...
such systems to become a reality. The key problem is \emph{type evolution}, i.e., how to
...
even implicit. The key property of types for us is that they determine the shape of other layers
...
We now describe a conceptual framework lets us view a range of diverse set of real-world
...
data that are structured according to types. In specific case studies, the layers may map to
...
\section{Live programming in the Elm architecture}
\label{SECT:elm}

The first challenge problem we recast in terms of our conceptual framework is live programming
...
discard state and restart the application when types are changed. A more effective programming
system based on live programming would allow updates to both the code of the two functions and
the structure of the two types.
...
\challenge{Live Type Evolution of State and Events}
...
require data migration (assuming that the change does not happen while there are unprocessed
...
default value) to challenging case. In particular, if a programer refactors \texttt{State} to
...
\textcolor{red}{TODO: The rest needs more editing}
...
Model Driven Engineering has also studied evolution. Unlike the textual artifacts involved in other domains, models typically assign unique indentifiers enabling more precise differencing and mergeing~\cite{alanen2003}.
...
\citet{wang19} use program synthesis to rewrite SQL programs to adapt to a schema change. This process is driven by finding possible correspondences between columns in the old and new schema guided by hueristics on textual similarity. It is assumed the migration does not change datatypes or lose any information visible to the programs yet those are real needs. It is not clear how pathologically ambiguous schema changes are resolved (for example renaming equityped columns).

Programming by example has been used to infer schema migration programs~\cite{wang20, Alexe11}. This work has dealt with the reality that schema live in the database and so examples must be supplied separately, but we wonder why not incorporate examples into a schema definition language? Then purely textual edits that properly adapted the examples could be used to infer migrations and resolve the ambiguities from comparing pure schema.

\paragraph{Tijs} \cite{vanRozen15, vanRozen19, RuntimeConstraint, SemanticDeltas}

\paragraph{Live, MDD, etc. programming?}

% Some references from Tomas
\paragraph{collaborative programmign tools}
\cite{goldman2011real,kurniawan2015coder,replit}
\paragraph{exploratory/live review paper}
\cite{rein2018exploratory}
\paragraph{hot reload in live systems}
