...
~
\textcolor{red}{[JE] We could expand our context to include schema change for conventional DB systems, not just live/local. It is a widespread problem, attested by the many tools that assist with it, but all of them come down to manually coding SQL. We could call for a higher-level declarative solution. }
...
Schema change and data migration are a widespread problem in practice for SQL databases, attested by the many tools that have been created to help. Some tools, like Rails Migrations~\cite{RailsMigrations} provide a Domain Specific Language (DSL) for specifying and executing schema changes. Other tools, such as those in Liquibase~\cite{liquibase} or PlanetScale\cite{planetscale} are based on textually comparing SQL Data Definition Language (DDL) statements that define the database schema. The differences between these DDL schema can be used to infer a delta of DDL statements to change one into the other. Unfortunately these inferences must sometimes guess at the user's intent: for example has a column been renamed or has it been deleted and a new one created? The difference matters a lot for the data in that column! EdgeDB~\cite{edgedb} asks the developer to resolve such ambiguities with a series of questions.

All of these tools end up emitting SQL DDL statements to change the schema. Unfortunately that does not migrate the data. Some databases add limited features for migration. For example SQLite~\cite{sqliteDatatypes} uses dynamically typed values allowing them to be implicitly converted if the datatype of the column changes. MYSql~\cite{mysqlAlterTable} can reorder columns without destroying their data. But apart from such special cases, SQL databases have no general purpose support for schema migration. A developer must write custom SQL code to transform data from the old to the new schema. This custom migration code is greatly complicated if it needs to be done without taking the database offline or must be coordinated across multiple shards. In \textit{Refactoring Databases} \citet{ambler06} offer a comprehensive taxonomy of schema change patterns, including typical strategies for online migration and example SQL code.

There is surprisingly little discussion of schema migration in the database literature nor support from the major database vendors. There is indeed a rich theory of database normalization~\cite{Molina08} but that is presumed to happen before the database is populated with data. To a large extent this entire problem area has been relegated to application developers and `dev ops'. Yet prima facia, schema migration can only be done throughly with direct support inside the database. EdgeDB~cite{edgedb} stands out for its improved support of schema migration through a proprietary language of data transformations but is still limited by the capabilties of its underlying PostgresSQL database.

\begin{comment}

Newly discovered thread of DB research on schema change:
...
https://dl.acm.org/doi/10.14778/1453856.1453939
https://dl.acm.org/doi/10.1145/3448016.3452842
https://dl.acm.org/doi/10.14778/3565816.3565818
https://link.springer.com/chapter/10.1007/978-3-030-58135-0_10
...
Smalltalk images
	depends on dynamic types and adding new fields to objects
	Gemstone added complex API

“unstructured data”: JSON and RDF used with untyped PL like Javascript or Python
Like Smalltallk, small set of changes can be handled gradecfully, but in general need to write custom code

dataframe diff
https://homepages.inf.ed.ac.uk/csutton/publications/datadiff.pdf
...
Geoffreu on Local-first
https://www.inkandswitch.com/cambria/
https://riffle.systems/papoc22.pdf
