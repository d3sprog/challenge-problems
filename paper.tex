...
\citet{wang19} use program synthesis to rewrite SQL programs to adapt to a schema change. This process is driven by finding possible correspondences between columns in the old and new schema guided by heuristics on textual similarity. It is assumed the migration does not change datatypes or lose any information visible to the programs yet those are real needs. It is not clear how pathologically ambiguous schema changes are resolved (for example renaming equityped columns).
