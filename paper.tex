...
we will return to repeatedly in this paper. When a type changes from $\textsf{type}_1$ to $\textsf{type}_2$,
...
applying changes that will update $\textsf{data}_1$ and $\textsf{code}_1$ into new versions $\textsf{data}_1$ and $\textsf{code}_2$
...
\begin{figure}[t]
\centering
%\includegraphics[scale=0.175]{figures/diag-data-code.png}
\begin{tikzcd}[sep = 6em]
  \textsf{code}_1
  \arrow[d, "\textsf{refactor}"{name=L, description}]
  &
  \textsf{type}_1
  \arrow[l, "\textsf{constrains}"{above}]
  \arrow[r, "\textsf{constrains}"{above}]
  \arrow[d, red, "\textsf{change}"{name=C, description}]
  &
  \textsf{data}_1
  \arrow[d, "\textsf{migrate}"{name=R, description}]
  \\
  \textsf{code}_2
  &
  \textsf{type}_2
...
\end{tikzcd}
\caption{When a type changes from $\textsf{type}_1$ to $\textsf{type}_2$, a corresponding change needs to take place
  in data and code constrained by the type. The challenge is finding a suitable
...
% \vspace{1em}
% \includegraphics[scale=0.175]{figures/diag-both.png}
% \end{wrapfigure}
\frameworkboxtitle{Code-data Co-evolution}
In this case study, the evolving type in Figure~\ref{fig:change} is the \texttt{State} type. When the type
evolves, corresponding change needs to be applied to both \emph{code} and \emph{data}.
The former is the program logic implemented in \texttt{update} and \texttt{render},
while the latter is the current state (a live value of type \texttt{State}). The scenario
illustrates that code and data need to co-evolve as illustrated by a revised diagram on the right.
}
...
\textcolor{red}{[JE. Found this snippet in the related work]
...
\begin{tikzcd}[sep = 6em]
  \textsf{type}_1
  \arrow[r, "\textsf{constrains}"{above}]
  \arrow[d, red, "\textsf{change}_1"{name=C, description}]
  &
  \textsf{data}_1
  \arrow[d, "\textsf{migrate}_1"{name=R, description}]
  \\
  \textsf{type}_2
  \arrow[r, "\textsf{constrains}"{above}]
  \arrow[d, red, "\textsf{change}_2"{name=C2, description}]
  &
  \textsf{data}_2
  \arrow[Rightarrow, from=C, to=R, "\textsf{propagate}"{above}]
  \arrow[d, "\textsf{migrate}_2"{name=R2, description}]
  \\
  \textsf{type}_3
  \arrow[r, "\textsf{constrains}"{above}]
  &
  \textsf{data}_3
  \arrow[Rightarrow, from=C2, to=R2, "\textsf{propagate}"{above}]
\end{tikzcd}
\caption{Multiversion type evolution.}
\label{fig:multiversion}
\end{figure}

\frameworkbox{
% wrapfigure doesn't seem to work with tikz
% \begin{wrapfigure}{r}{9em}
% \includegraphics[scale=0.2]{figures/diag-variants.png}
% \end{wrapfigure}

\frameworkboxtitle{Primitive Type Transformations}
In this example, \textsf{type} in Figure~\ref{fig:multiversion} refers to data tables, but the same scenario applies to
objects or JSON values in other programming systems. Our example focuses on permanent \emph{data}
that need to evolve alongside with the type and keep synchronized even in the non-local scenario.
The evolution can be done through a small set of primitive type transformations (illustrated on the
right) with corresponding data transformation for each. The system may either expose the
primitive transformations through a user interface, or infer them from textual edits made by the
programmer, e.g. to a database table schema definition.
}
...
Jennings). The first challenge involves merging two independent edits to the data
(Fig.~\ref{fig:conf-add} and Fig.~\ref{fig:conf-tab}); the second merging structural change
with code (Fig.~\ref{fig:conf-budget} and Fig.~\ref{fig:conf-tab}). Formulas in the final version
(Fig.~\ref{fig:conf-finfin}) are updated to match the new document structure.}
\label{fig:conf}
\end{figure}
...
\item Tomas refactors the list into a table. He splits the single textual value
  ...
  This changes the document structure (type).
\end{enumerate}
...
``Organizer'' column, the system may use a suitable default value or ask the user interactively.

\frameworkbox{
\begin{wrapfigure}{r}{8em}
\includegraphics[scale=0.25]{figures/diag-data-change.png}
\end{wrapfigure}
\frameworkboxtitle{Independent Data and Code Edits}
In this example, the \emph{type} is implicit and represents the structure of the document.
A change to the \emph{type} done by one organizer needs to be applied to \emph{data} that
has been modified by the other organizer in the meantime, as illustrated on the right.
In the next section, we will see similar pattern, but with formulas, or \emph{code}.
We assume the \emph{decentralized} scenario where \emph{type} evolution can occur in any variant
and \emph{covergence} model where all variants eventually adopt all changes. As discussed
later, the system could also operate in the more challenging \emph{divergence} model.}
...
upgrades, teams must manually perform multi-step zero-downtime deployment processes~\cite{planetscale}
...
\vspace{0.5em}
\includegraphics[scale=0.25]{figures/diag-divergence.png}
\end{wrapfigure}
...
% A special case of live programming is \emph{hot reloading} or \emph{hot swapping})~\cite{barenz2020essence, hicks2005dynamic} that installs code changes into a running system preserving state but not attempting to migrate that state through type changes.
