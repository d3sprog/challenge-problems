...
% \includegraphics[scale=0.175]{figures/diag-variants.png}
%\end{wrapfigure}
...
with embedded computed values. In our model, a computed value is a document node whose value is
specified by a simple formula (akin to those in Potluck~\cite{Litt2023} or spreadsheets) that
may refer to other nodes of the document. We assume that documents can be concurrently edited
by multiple users in a local-first manner, i.e., local edits are later synchronized with peers.

Our concrete example is collaborative conference planning, illustred in Figure~\ref{fig:conf}
...
Tomas. The newly added speaker should be reformatted into the new format. For the newly added
``Organizer'' column, Tomas may specify default value or the newly added row may use an
empty value.

\frameworkbox{
\frameworkboxtitle{Framework perspective}
