...
both in collaboration with other programmers (as in collaborative development tools
\cite{goldman2011real,kurniawan2015coder,replit}) and in individual's interaction with the
programming environment (as in live programming \cite{rein2018exploratory}).

To further tighten the feedback loop and make programming more effective, future programming
systems will need to enable programmers to collaborate at a fine-grained level, letting them
adopt individual changes created by their collaborators (as in local-first software
\cite{localfirst}). To provide immediate feedback, future programming systems will need to
transcend treating state as ephemeral, tied to the lifetime of a running program and reset when the
program is edited.

We need to shift our focus to \emph{programming systems} \cite{techdims} where program state
is persistent and co-evolves with the behavior and data of the system. This is already the case
in collaborative applications with state persistent in a database, image-based systems like
Smalltalk and live programming environments with hot-reloading where the program can be edited
...
such systems to become a reality. The key problem is of \emph{type evolution}, i.e., how to
handle the change of schema of the data program works with in a way that preserves integrity
...
In the database world, the problem of schema evolution is well-studied \cite{erhard06}.
It typically focuses on ensuring data integrity, although co-evolution of database structure
and code that manipulates the database has also been studied \cite{Cleve2006,wang2019synthesizing}.
...
data migrations. The problem is equally studied in the bidirectional transformation (BX)
communities \cite{czarnecki2009bidirectional} and in work on model-driven development
\cite{metamodels,alanen2003difference}.

Type evolution is an important challenge for future programming systems. But looking at the problem
from the perspective of programming systems can also offer a new perspective that
...
essential shared structure. By doing so, we aim to motivate further research on this important
problem and encouraging knowledge sharing across subfields. Our contributions are twofold.
First, we present a shared conceptual framework for thinking about the relationship between
schema, code, and data in a programming system. Second, we present five case studies that use
the framework to study type evolution in a highly diverse set of real-world programming scenarios.
...
in response to new needs. As with buildings (Figure~\ref{fig:layers}), different aspects of
program evolve at different pace. And ``because of the different rates of change of its components,
a program is always tearing itself apart.''

\subsection{Types, Code and Data}

As is the case for buildings, making a change at some layers (skin or stuff) is relatively easy,
but a change at a more basic layer (structure) has cascading effects on layers that depend on it.
In the case of programs, a more basic layer is a structure that defines the shape of data and code.
We refer to this structure as \emph{types}, although we do not use the term in an precise technical
...
even implicit. The key property of types for us is that they define the shape of other layers
of a program, such as data and code. As a result, a change of program types requires a
...
may be that the types and schema of a program should converge, but data may diverge.

\begin{figure}[t]
\centering
\includegraphics[width=0.65\textwidth]{figures/arrows.png}
...
\textcolor{red}{TODO: got as far as here so far...}
...
\section{diagrams}
  ...
  \begin{tikzcd}[sep = huge]
    \textsf{code}_1
    \arrow[d, "\textsf{refactor}"{left},""{name=L}]
    &
    \textsf{schema}_1
    \arrow[l, "\textsf{constrains}"{above}]
    \arrow[r, "\textsf{constrains}"]
    \arrow[d, red, "\textsf{change}"{near start, description}, ""{name=CL, left}, ""{name=CR}]
    &
    \textsf{data}_1
    \arrow[d, ""{name=R, left}, "\textsf{migrate}"{right}]
    \\
    \textsf{code}_2
    &
    \textsf{schema}_2
    \arrow[l, "\textsf{constrains}"{below}]
    \arrow[r, "\textsf{constrains}"{below}] &
    \textsf{data}_2
    \arrow[Rightarrow, from=CL, to=L, "\textsf{propagate}"{below}]
    \arrow[Rightarrow, from=CR, to=R, "\textsf{propagate}"{below}]
  \end{tikzcd}
...
\paragraph{Live, MDD, etc. programming?}


% Some references from Tomas
%
% collaborative programmign tools
\cite{goldman2011real,kurniawan2015coder,replit}
% exploratory/live review paper
\cite{rein2018exploratory}
% hot reload in live systems
\cite{barenz2020essence,beckmann2021shortening}

% Some references from Tijs
%
% SQL+code
\cite{Cleve2006,wang2019synthesizing}
% BX
\cite{czarnecki2009bidirectional}
% MDD
\cite{metamodels,alanen2003difference}.
