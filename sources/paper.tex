...
write alone, it is impossible to make a perfect choice among these options. A programming system could consult the user to disambiguate their intent, as well as choose (and explain) a default behavior.
...
for Extract Entity. The initial data and schema $D_1, S_1$ co-evolve into $D_2, S_2$ (adding
...
generated for the schema evolution (from $S_1$ to $S_2$) cannot be generated fully automatically.
...
\begin{minipage}{0.3\textwidth}
  \begin{lstlisting}[language=java,morekeywords={machine,on,state,var},basicstyle=\footnotesize,numbers=none,xleftmargin=1.5em]
machine Doors
...
Remove & variable & Remove from the variable map (assuming it is unused)\\
...
\paragraph{Data Wrangling} The evolution of data formats is a concern in Data Science. The Data~Diff tool~\cite{sutton18} compares two versions of tabular data and finds a minimal matching transformation. AI~assistants~\cite{AIassistants} provide a mechanism for interactively correcting
